...
# Persona names and interface text intentionally use Kazakh-facing defaults.
PERSONA_SPECS = (
    PersonaSpec(
        "resident_owner", "🏠 Тұрғын · меншік иесі", "Айдана Қасым",
        "resident", 1, apartment="D-01", resident_subrole="owner",
    ),
    PersonaSpec(
        "resident_tenant", "🔑 Тұрғын · жалға алушы", "Нұрлан Әли",
        "resident", 2, apartment="D-01", resident_subrole="tenant",
    ),
    PersonaSpec(
        "worker_electrician", "⚡ Электрик", "Әлихан Серік",
        "worker", 3, worker_category="electrician", is_on_shift=True,
    ),
    PersonaSpec(
        "worker_plumber", "🔧 Сантехник", "Мақсат Омар",
        "worker", 4, worker_category="plumber", is_on_shift=True,
    ),
    PersonaSpec(
        "worker_security", "🛡️ Күзет", "Санжар Бек",
        "worker", 5, worker_category="security", is_on_shift=True,
    ),
    PersonaSpec(
        "worker_cleaning", "🧹 Клининг", "Аружан Нұр",
        "worker", 6, worker_category="cleaning", is_on_shift=True,
    ),
    PersonaSpec(
        "worker_kazakhdomofon", "📹 Қазақдомофон", "Данияр Есен",
        "worker", 7, worker_category="kazakhdomofon", is_on_shift=True,
    ),
    PersonaSpec(
        "dispatcher", "🎛️ Диспетчер", "Меруерт Асқар",
        "dispatcher", 8,
    ),
    PersonaSpec(
        "administrator", "🛡️ Төраға", "Ержан Төлеу",
        "administrator", 9,
    ),
)
...
def _apply_spec(user: User, spec: PersonaSpec) -> None:
...
async def _get_or_create_persona(
    session: AsyncSession, controller_telegram_id: int, spec: PersonaSpec
) -> DevPersona:
...
def dev_keyboard(current: str | None) -> InlineKeyboardMarkup:
    builder = InlineKeyboardBuilder()
    own_prefix = "✅ " if current is None else ""
    builder.button(text=f"{own_prefix}👤 Өз профилім", callback_data="dev_switch:self")
    for spec in PERSONA_SPECS:
        prefix = "✅ " if spec.key == current else ""
        builder.button(
            text=f"{prefix}{spec.label}", callback_data=f"dev_switch:{spec.key}"
        )
    builder.adjust(1, 2, 2, 2, 2, 1)
    if current is not None:
        rows = list(builder.export())
        rows.append([InlineKeyboardButton(
            text="🧹 Осы персонаның деректерін тазарту",
...
async def dev_entry(message: Message, state: FSMContext, session: AsyncSession):
    if not DEV_MODE:
        await message.answer("⛔ DEV_MODE өшірулі.")
        return
    await state.clear()
    ...
    if real_user is None:
        await message.answer("Алдымен /start пәрменін жіберіңіз.")
        return
    active = await _active_persona(session, message.from_user.id)
    current = active[0].persona_key if active else None
    current_label = PERSONAS_BY_KEY[current].label if current else "Өз профилім"
    await message.answer(
        "🔧 <b>DEV персоналар</b>\n"
        f"Белсенді: <b>{current_label}</b>\n\n"
        "Тестілеу үшін персонаны таңдаңыз. Әр персона тұрақты және өзінің "
        "өтінімдер тарихын сақтайды.",
        parse_mode="HTML",
        reply_markup=dev_keyboard(current),
    )
...
async def dev_switch(callback: CallbackQuery, session: AsyncSession):
    if not DEV_MODE:
        await callback.answer("DEV_MODE өшірулі", show_alert=True)
        return
    controller_id = callback.from_user.id
    if await session.scalar(
        select(User.id).where(User.telegram_id == controller_id)
    ) is None:
        await callback.answer("Алдымен /start пәрменін жіберіңіз", show_alert=True)
        return
    ...
    if choice == "self":
        ...
        user = await session.scalar(select(User).where(User.telegram_id == controller_id))
        label = "Өз профилім"
        current = None
    else:
        spec = PERSONAS_BY_KEY.get(choice)
        if spec is None:
            await callback.answer("Белгісіз персона", show_alert=True)
            return
        persona = await _get_or_create_persona(session, controller_id, spec)
    ...
    await callback.message.edit_text(
        f"✅ Белсенді персона: <b>{label}</b>",
        parse_mode="HTML",
        reply_markup=dev_keyboard(current),
    )
    if user is not None:
        await callback.message.answer(
            f"Басты мәзір · {label}", reply_markup=get_main_keyboard(user)
        )
    await callback.answer("Персона ауыстырылды")
...
async def dev_persona_reset(callback: CallbackQuery, session: AsyncSession):
    ...
    if active is None:
        await callback.answer("Белсенді DEV персона жоқ", show_alert=True)
        return
    persona, user = active
    ...
    _apply_spec(user, spec)
    await session.commit()
    await callback.answer("Персона деректері тазартылды", show_alert=True)
...
async def dev_reset(message: Message, state: FSMContext, session: AsyncSession):
    ...
    if user is None:
        await session.commit()
        await message.answer("Профиль жоқ — /start пәрменін жіберіңіз.")
        return
    uid = user.id
    try:
        ...
        if user is None:
            await session.commit()
            await message.answer("Профиль жойылды. /start пәрменін жіберіңіз.")
            return
        user.full_name = None
        user.apartment = None
        ...
        await session.commit()
        await message.answer(
            "⚠️ Профиль бастапқы күйге қайтарылды. Қайта тіркелу үшін /start "
            "пәрменін жіберіңіз."
        )
        return
    await message.answer(
        "🗑️ Профиль жойылды. Қайта тіркелу үшін /start пәрменін жіберіңіз."
    )
